        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if (empty($puestos)): ?>
                    <tr><td colspan="5" class="px-6 py-8 text-center text-gray-500">No hay puestos</td></tr>
                <?php else: ?>
                    <?php foreach ($puestos as $puesto): ?>
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                <?php echo htmlspecialchars($puesto['departamento_nombre'] ?? 'Sin asignar'); ?>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                <button onclick="editarPuesto(<?php echo $puesto['id']; ?>)" 
                                        class="text-blue-600 hover:text-blue-900 mr-3">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="eliminarPuesto(<?php echo $puesto['id']; ?>, '<?php echo htmlspecialchars($puesto['nombre'], ENT_QUOTES); ?>')" 
...
<div id="modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <h3 id="modal-title" class="text-lg font-semibold text-gray-900 mb-4">Nuevo Puesto</h3>
        <form id="form-puesto" onsubmit="guardar(event)">
...
<script>
function abrirModal() {
    document.getElementById('modal-title').textContent = 'Nuevo Puesto';
    document.getElementById('form-puesto').reset();
...
function editarPuesto(id) {
    fetch(BASE_URL + 'catalogos/obtener-puesto?id=' + id)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('departamento_id').value = data.data.departamento_id || '';
...
function guardar(event) {
    ...
    fetch(BASE_URL + 'catalogos/guardar-puesto', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function eliminarPuesto(id, nombre) {
    if (confirm('¿Eliminar puesto "' + nombre + '"?')) {
        fetch(BASE_URL + 'catalogos/eliminar-puesto', {
            method: 'POST',
